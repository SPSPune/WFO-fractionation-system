...
def get_latest_sql_timestamp(sql_server, sql_db_name, sql_table_name):
    ...
    conn = None
    try:
        # Use a trusted connection string
...
def sync_continuously(config, tag_mapping, password):
    ...
    while st.session_state.sync_running:
        ...
        try:
            ...
            _log_message(f"ℹ️ Connecting to SQL Server...")
...
# --- DB Setup ---
if submitted:
    st.session_state.pg_password = password # Save the password securely in session state
    if create_database_if_not_exists(host, port, user, st.session_state.pg_password, db_name):
        create_pivoted_table_if_not_exists(host, port, user, st.session_state.pg_password, db_name, pg_table_name)
...
# The log area is a key part of the diagnostic
sync_log_container = st.container()
with sync_log_container:
    st.text_area("Log Output", "\n".join(st.session_state.sync_log), height=400)

# This is a crucial line for updating the log in real-time.
if st.session_state.sync_running:
    time.sleep(1) # Rerun every second to update the log
    st.rerun()
